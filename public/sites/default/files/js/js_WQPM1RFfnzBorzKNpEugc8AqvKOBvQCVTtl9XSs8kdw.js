...
function ZakatCalc() {
    var txtval = $('input[id*="txtAmount"]').val();

    if (txtval.length == 0 || txtval == '' || txtval == undefined)
        $('span.DevHelperValCtrl').css('display', 'block');
    else {
        $('span.DevHelperValCtrl').css('display', 'none');
        var result = Math.round(txtval / 40);
        $('input[id*="txtResult"]').val(result);
    }
}
function updatecart() {
    $.ajax({
        type: "POST",
        url: js_path + 'store/cart/rendar',
    ...
    $.ajax({
        type: "POST",
        url: js_path + 'store/cart/rendarI',
        success: function (msg) {
            $('.cartDataI2').html(msg);
        }
    });
}
function addtocart(qid, ctype, kind) {
    var count = $('.payCount' + kind + ' :selected').val();
//        alert(qid + '-' + ctype + '-' + kind + '-' + count);

//    if (confirm('هل أنت متأكد؟')) {
    $.ajax({
        type: "POST",
        url: js_path + 'store/cart/add/' + qid + '/' + ctype + '/' + kind + '/' + count,
        success: function (msg) {
            if (msg == 1) {
//                    updatecart();
                window.location.href = js_path + "pages/16";
...
function addtocart1(qid, ctype, kind) {
    var count = 1;
...
//    if (confirm('هل أنت متأكد؟')) {
    $.ajax({
        ...
        success: function (msg) {
            if (msg == 1) {
//                    updatecart();
                window.location.href = js_path + "store/cart";
//                    alert('تمت الإضافة للسلة');
            } else {
//                    alert('لم تضف للسلة');
            }
...
function delfromcart(key) {
    ...
    if (confirm('هل أنت متأكد؟')) {
        $.ajax({
            type: "POST",
            url: js_path + 'store/cart/del/' + key,
            success: function (msg) {
                updatecart();
            }
        });
...
$(document).ready(function () {
    ...
    $("input[name='showdate']").datepicker({dateFormat: 'dd/mm/yy', changeMonth: true, changeYear: true, yearRange: "1940:2200"});
    $("input[name='hidedate']").datepicker({dateFormat: 'dd/mm/yy', changeMonth: true, changeYear: true, yearRange: "1940:2200"});

    $("input[name='showdateX']").datepicker({dateFormat: 'dd/mm/yy', changeMonth: true, changeYear: true, maxDate: "+1M +10D", yearRange: "1940:2200"});
    $("input[name='bdate']").datepicker({dateFormat: 'dd/mm/yy', changeMonth: true, changeYear: true, maxDate: "+1M +10D", yearRange: "1940:2200"});
